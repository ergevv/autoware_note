...
{
  const std::string yaml_filename = this->declare_parameter<std::string>(
    "map_projector_info_path");  // autoware_map/sample-map-planning/map_projector_info.yaml
  const std::string lanelet2_map_filename = this->declare_parameter<std::string>(
    "lanelet2_map_path");  // autoware_map/sample-map-planning/lanelet2_map.osm

  const autoware_map_msgs::msg::MapProjectorInfo msg =
    load_map_projector_info(yaml_filename, lanelet2_map_filename); //读取地图投影信息和缩放因子
